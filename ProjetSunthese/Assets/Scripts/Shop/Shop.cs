...
public class Shop : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            int nb = Random.Range(0, possibleOtherItem.Count);
            GameObject item = Instantiate(possibleOtherItem[nb], stands[i].transform);
            possibleOtherItem.RemoveAt(nb);
    ...
    private void PrepareItem(GameObject item, int standIndex)
    {
        ...
        item.transform.GetChild(0).gameObject.SetActive(false);
        item.AddComponent<ShopItemMovement>();
        stands[standIndex].GetComponent<ShopItem>().SetItem(item, basePrice);
        item.SetActive(true);
    }
}
